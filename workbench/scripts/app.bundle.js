...
const app = createApp({
  setup() {
    ...
    async function saveTodo() {
      ...
      const record = { id: todoForm.id || crypto.randomUUID(), title: todoForm.title.trim(), note: todoForm.note.trim(), date: todoForm.date, dueAt: todoForm.dueAt ? new Date(todoForm.dueAt).toISOString() : '', priority: normalizePriority(todoForm.priority), completed: existing?.completed || false, completedAt: existing?.completedAt || null, createdAt: existing?.createdAt || stamp, updatedAt: stamp };
